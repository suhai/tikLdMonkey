export const CheekChubby = [
  {
    id: "1",
    body: "This is a Ruby on Rails application that was designed for users to access posts or Articles on various topics of their interests. In addition there is a section for Photos where users can scroll through photos by category. Users can search for keywords to narrow down the list of displayed posts or photos in alphabetical order. Users have the option to just read the articles, browse through images, or create their own posts and or upload images for other users to access. To post an article or create a post or upload an image to the photos section, a user would have to sign up and login. Currently, a a logged in user only has Create and Read privilges, with the option to be upgraded to administrator position if they have more content to share and desires to be designated as admin. A wysiwyg editor is made available to help the user format their text in addition to having the ability to uplaod images or videos with the editor. For the posts section, the paperclip gem was used for image uploads while the carrierwave gem was used for image uploads for the photos section. I experimented with persisting uploaded images to AWS database but opted to just use a default image url for uploaded images that get wiped by heroku after being uploaded. I eventually plan to set up an AWS S3 bucket to store and serve uploaded images. I am also currently working on the comments section and once that is completed, users should be able to comment on articles / posts and or photos, with references to their credentials stored with their comments in the database. I also plan to configure AWS S3 buckets to store image files uploaded by users from the app, or even probably migrate the entire app from Heroku to AWS.",
